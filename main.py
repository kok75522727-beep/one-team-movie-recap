...
from media_engine import generate_script, generate_tts, probe_video, render_final
...
def resolved_api_key(request: JobRequest, entitlement: Entitlement) -> str:
    if entitlement.plan == "simple":
        key = request.simple_api_key.strip()
        if not key:
            raise RuntimeError("Simple Trial အတွက် ကိုယ့် Gemini API Key ထည့်ပါ")
        return key
    key = os.getenv("ONE_TEAM_VIP_GEMINI_API_KEY", "").strip()
    if not key:
        raise RuntimeError("VIP Gemini server key မသတ်မှတ်ရသေးပါ")
    return key
...
def process_job(job: ExportJob, request: JobRequest) -> None:
    try:
        ...
        narration = generate_tts(api_key, script, str(settings.get("voice") or "Kore"))
        if cancelled(job):
            return
...
async def current_membership(authorization: str | None = Header(default=None)) -> dict[str, Any]:
    try:
        ...
        return {"plan": str(member.get("effective_plan") or "none"), "tier": str(member.get("subscription_tier") or ""), "trial_days_remaining": int(member.get("trial_days_remaining") or 0), "trial_expired": bool(member.get("trial_expired")), "is_admin": bool(member.get("is_admin")), "email": str(member.get("email") or "")}
